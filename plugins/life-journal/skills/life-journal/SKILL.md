---
name: life-journal
description: Initialize and maintain a local Markdown-based life journal containing dated diary entries, people, confirmed visits, media, thoughts, and reusable personal experiences. Use when the user wants to record, correct, organize, validate, browse, or start the local Web reader for their life records; do not use for generic literary journaling or public publishing without an explicit privacy review.
---
...
- A request to open or start the Web reader requires [references/web-reader.md](references/web-reader.md). Default to localhost or a private LAN; starting the reader does not authorize public exposure.
- A request to check or apply an update to an existing Vault requires [references/upgrades.md](references/upgrades.md). Skill updates and Vault migrations are separate operations.
...
- Preserve user-customized local rules when the Skill is upgraded. Never replace an existing Vault's `AI_GUIDE.md` with the bundled template.
...
- Read [references/upgrades.md](references/upgrades.md) before checking versions or migrating an existing Vault.
